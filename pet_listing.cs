    ...
    public partial class pet_listing : Form
    {
        ...
        private void add_button_listing_Click_1(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (Type.All(Char.IsLetter) == false)
                {
                    MessageBox.Show("Pet Type should contain only alphabets.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                if (pName.All(Char.IsLetter) == false)
                {
                    MessageBox.Show("Pet Name should contain only alphabets.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                if (Gender.All(Char.IsLetter) == false)
                {
                    MessageBox.Show("Gender should contain only alphabets.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                if (age.All(Char.IsDigit) == false)
                {
                    MessageBox.Show("Age should be integer.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                if (Number.All(Char.IsDigit) == false)
                {
                    MessageBox.Show("Number should be integer.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                if (IsValidFloat(price) == false)
                {
                    MessageBox.Show("Price should be float and positive.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
