...
namespace PetApp
{
    public partial class adoption_application : Form
    {
        ...
        public bool IsValidFloat(string str)
        {
            ...
            if (str.Length == startIndex) return false;
        ...
        private void submit_button_application_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if ((Name.All(Char.IsLetter) == false)|| (City.All(Char.IsLetter) == false) || (Town.All(Char.IsLetter) == false) || (other_pet.All(Char.IsLetter) == false) || (Home_type.All(Char.IsLetter) == false) || (Home_status.All(Char.IsLetter) == false) )
                {
                    MessageBox.Show("Some fields should be alphabets.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                if ((Number.All(Char.IsDigit) == false) || (Street.All(Char.IsDigit) == false) || (House.All(Char.IsDigit) == false) || (Children.All(Char.IsDigit) == false) || (Age.All(Char.IsDigit) == false) || (Pet_ID.All(Char.IsDigit) == false))
                {
                    MessageBox.Show("Some fields should be integer.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                if (!IsValidEmail(Email))
                {
                    MessageBox.Show("Email formate should be correct.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
                if (IsValidFloat(Income) == false)
                {
                    MessageBox.Show("Income should be float and positive.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
